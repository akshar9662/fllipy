...
export default function AddressForm({ onAddress, AddressData, onDelete, onSaveAddress }) {
  ...
  const SaveAddress = () => {
    ...
    const newAddress = { name, phone, address, city, state, pincode, addressType };
      ...
      <form className="border p-4 rounded shadow-sm">
        ...
        <div className="row mb-3">
          <div className="col-md-6">
            ...
            <input type="text" className="form-control" value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter your full name" required />
          </div>
          <div className="col-md-6">
            <label>Phone Number</label>
            <input type="number" className="form-control" value={phone} onChange={handlephoneChange} placeholder="Enter phone number" required />
          </div>
        </div>

        <div className="mb-3">
          <label>Address</label>
          <textarea className="form-control" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Enter full address" rows="2" required />
        </div>
        ...
        <div className="row mb-3">
          ...
          <div className="col-md-4">
            <label>State</label>
            <input type="text" className="form-control" value={state} onChange={(e) => setState(e.target.value)} placeholder="Enter state" required />
          </div>
          <div className="col-md-4">
            <label>Pincode</label>
            <input type="number" className="form-control" value={pincode} onChange={handlepincodeChange} placeholder="Enter pincode" required />
          </div>
        </div>
    ...
    {["Home", "Work", "Other"].map((type) => (
      <div key={type} className="form-check form-check-inline">
        <input
          ...
          id={`addressType-${type}`}
        />
        <label className="form-check-label" htmlFor={`addressType-${type}`}>
          {type}
        </label>
      </div>
    ))}
            ...
            <tbody>
              {AddressData.map((data, index) => (
                <tr key={index}>
                  ...
                  <td><button className="btn btn-warning btn-sm" onClick={() => onEdit(index)}>✏️</button></td>
                  <td><button className="btn btn-danger btn-sm" onClick={() => onDelete(index)}>🗑️</button></td>
                </tr>
              ))}
            </tbody>
          </table>
